...
export ARO_MASTER_SIZE="{ARO_MASTER_SIZE:="Standard_D32s_v5"}"
export ARO_WORKER_SIZE="{ARO_WORKER_SIZE:="Standard_D8s_v5"}"
export ARO_WORKER_COUNT="{ARO_WORKER_COUNT:=3}"
export ARO_REQUIRED_REPLICAS="{ARO_REQUIRED_REPLICAS:=8}"
export ARO_INFRA_SIZE="{ARO_INFRA_SIZE:="Standard_E16s_v5"}"
export ARO_UPGRADE_OCP="{ARO_UPGRADE_OCP:="True"}"
export VERSION="${VERSION:-$(az aro get-versions --location ${LOCATION} -o json | jq '.[-1]' -r)}"
...
aro_verify_permissions() {
    ...
    az vm list-usage -l $LOCATION --query "[?contains(name.value, 'standardDSv5Family')]" -o table

    echo "INFO: Current Subscription ESv5 Quota for Infrastructure Nodes"
    az vm list-usage -l $LOCATION --query "[?contains(name.value, 'standardESv5Family')]" -o table
...
aro_scale(){
    ...
    while [ "$all_running" == false ]; do
        ...
        machines_info=$(oc get machines -o json)
